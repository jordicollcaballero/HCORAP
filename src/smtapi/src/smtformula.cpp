...
void SMTFormula::addALK(const std::vector<literal> & v, int K, CardinalityEncoding enc){
	int n = v.size();
...
void SMTFormula::addAMOPB(const std::vector<std::vector<int> > & Q, const std::vector<std::vector<literal> > & X, int K, AMOPBEncoding encoding){
	...
	switch(encoding){
		...
		case AMOPB_GLBM:
			addAMOPBNaiveLocalPolynomialWatchdog(Q2,X2,K,true);
			break;

		case AMOPB_GLPW:
			addAMOPBLocalPolynomialWatchdog(Q2,X2,K);
			break;
...
void SMTFormula::addAMKSequentialCounter(const std::vector<literal> & x, int K){
	...
	for(int i = 0; i < n; i++){
		...
		if(i < n-1)
			addClause(!x[i] | Sout[0]);

		if(i>0 && i < n-1)
			for(int k = 0; k < K-1; k++)
				addClause(!Sin[k] | !x[i] | Sout[k+1]);

		if(i>0)
			addClause(!Sin[K-1] | !x[i]);
	}
}
...
void SMTFormula::addAMOPBLocalPolynomialWatchdog(const std::vector<std::vector<int> > & Q, const std::vector<std::vector<literal> > & X, int K)
{
	int n = Q.size();
	...
	for(int i = 0; i < Q.size(); i++){
		...
		for(int j = 0; j < Q[i].size(); j++){
			...
			//If K becomes 0, the encoding is simpler
				...
				for(int i2= 0; i2 < Q.size(); i2++) if(i2!=i){
					for(int j2 = 0; j2 < Q[i2].size(); j2++){
						addClause(!X[i][j] | !X[i2][j2]);
					}
				}
				continue;
			}
...
void SMTFormula::addAMOPBNaiveLocalPolynomialWatchdog(const std::vector<std::vector<int> > & Q, const std::vector<std::vector<literal> > & X, int K, bool useSorter){
	if(K==0){
		for(const std::vector<literal> & v : X)
			for(const literal & l : v)
				addClause(!l);
...
void SMTFormula::addAMOPBReducedGeneralizedGeneralizedTotalizer(std::vector<std::vector<int> >  Q, std::vector<std::vector<literal> > X, int K, bool reduce){
    ...
    while(repeatReduce) {
        ...
        // FUO Profiling to check how much reduction is happening at different
      	// depths
      	//long int fuo_num_values = 0;
      	//long int fuo_num_intervals = 0;

      /*	std::cerr << "RGGT node depth: 1, values: ";
      	std::cerr << tree[tree_size-1].values.size();
      	std::cerr << ", intervals: " << tree[tree_size-1].intervals.size();
      	std::cerr << std::endl;*/
        ...
        for(int i=tree_size-2; i>=0; i--) {
			...
			/*std::cerr << "RGGT node depth: " <<  tree[i].fuoGetDepth();
			std::cerr << ", values: " << tree[i].values.size();
			std::cerr << ", intervals: " << tree[i].intervals.size() << std::endl;
        	fuo_num_values += tree[i].values.size();
        	fuo_num_intervals += tree[i].intervals.size();*/
        }
        ...
        for(int i=n-1; i>=0; i--) {
            if(tree[i].intervals.size()==1) {
                ...
                X.erase(X.begin()+i);
                repeatReduce=true;
                //std::cerr << "RGGT: Aha - got one!!!" << std::endl;
            }
    ...
    int maxdepth=1;
   	for(int i=tree_size-1; i>=0; i--) {
        int depth = tree[i].fuoGetDepth();
        if(depth>maxdepth)
        	maxdepth=depth;
    }
    std::cerr << n <<";" << maxdepth << std::endl;
    return;
            ...
            for(int k = 0; k < r->intervals.size(); k++) {
	            ...
	            int lidx=0;
               	...
               	while(lidx <= ridx && !found){
               		paridx=(lidx+ridx)/2;
               		if(x < parint[paridx].first)
               			ridx = paridx-1;
               		else if(x > parint[paridx].second)
               			lidx = paridx+1;
               		else
               			found=true;
               	}
               	...
               	if(!found){
               		std::cerr << "Bad interval construction in RGGT" << std::endl;
					exit(-1);
               	}

	            addClause(!l->literals[j] | !r->literals[k] |  tree[i].literals[paridx]);
            }
        }
    }
...
void SMTFormula::addGerarquicAMOAMK(const std::vector<std::vector<std::vector<int> > > &Q,
		const std::vector<std::vector<std::vector<literal> > > & X,
		const std::vector<int> & capacities){

	if(Q.size()!=X.size() || Q.size() != capacities.size()){
		std::cerr << "Wrong AMO encoding" << std::endl;
		exit(BADCODIFICATION_ERROR);
	}
	//Depth of the gerarquy
	...
	if(n==1){
		addAMOPB(Q[0],X[0],capacities[0],AMOPB_GSWC);
		return;
	}

	//Compute cumulative capacities
	std::vector<int> cumulcap(n);
	cumulcap[0]=capacities[0];
	for(int i = 1; i < n; i++)
		cumulcap[i]=cumulcap[i-1]+capacities[i];

	//Retrieve outputs of sorter cirquits
	std::vector<std::vector<literal> > sorted(n);
	for(int i = n-1; i >= 0; i--)
		addAMOPBCardialitySWC(Q[i],X[i],sorted[i],cumulcap[i]);
	

	//Create merger circuits
	std::vector<literal> merged;
	for(int i = n-1; i >= 0; i--){
		

		if(merged.size() > capacities[i]){

			//Remove as many outputs as capacity of level i. The number of inputs is already truncated to the cumulative capacity
			std::vector<literal> merge_linput(merged.size()-capacities[i]);
			for(int j = 0; j < merged.size()-capacities[i]; j++)
				merge_linput[j] = merged[j+capacities[i]];

			addQuadraticMergeCardinality(merge_linput,sorted[i],merged,cumulcap[i]);
		}

		else //The previous levels are for sure supplied for the available capacities
			//Copy the input coming from the sorter, which is already truncated to the cumulative capacity
			merged=sorted[i];
		
	}
}

void SMTFormula::addAMOPBCardialitySWC(const std::vector<std::vector<int> > & Q, const std::vector<std::vector<literal> > & X, std::vector<literal> & output, int K){

	int N = X.size();

	std::vector<int> maxsum(N,0);

	//Compute the maximum output of each circuit
	for(int i = 0; i < N; i++)
		for(int q : Q[i])
			if(q>maxsum[i])
				maxsum[i]=q;
	...
	for(int i = 0; i < N; i++){
		...
		for(int j = 0; j < X[i].size(); j++){
			...
			if(i>0)
				for(int k = 0; k < osize-q; k++)
					addClause(!Sin[k] | !X[i][j] | Sout[k+q]);

			if(i>0 && q > 0 && K-q < isize)
				addClause(!Sin[K-q] | !X[i][j]);
		}
	}
	Sout.resize(osize);
...
void SMTFormula::addQuadraticMergeCardinality(const std::vector<literal> &x1, const std::vector<literal> &x2, std::vector<literal> &y, int K){
	if(x1.empty() || x2.empty()){
		...
		int addedSize = x1.size() + x2.size();
		int newSize = K < addedSize ? K : addedSize;
		y.resize(newSize);
		for(int i = 0; i < newSize; i++)
			y[i] = newBoolVar();

		int minX1NewSize = x1.size() < newSize ? x1.size() : newSize;
		int minX2NewSize = x2.size() < newSize ? x2.size() : newSize;

		for(int i = 0; i < minX1NewSize; i++){
			addClause(!x1[i] | y[i]);
			for(int j = 0; j < minX2NewSize; j++){
				if(i+j+1 < newSize)
					addClause(!x1[i] | !x2[j] | y[i+j+1]);
				else
					addClause(!x1[i] | !x2[j]);
		...
		for(int i = 0; i < minX2NewSize; i++)
			addClause(!x2[i] | y[i]);

		for(int i = minX1NewSize; i < x1.size(); i++)
			addClause(!x1[i]);

		for(int i = minX2NewSize; i < x2.size(); i++)
			addClause(!x2[i]);
	}
}
